...
- (void)layoutSubviews{
    CGFloat margin = 1;
    CGFloat H = self.contentView.frame.size.height - margin;
    UIImageView *lastImageView = [[UIImageView alloc]initWithFrame:CGRectMake(margin, margin,0,0)];
	for (int i = 0; i < [_rowAssets count]; ++i) {
		UIImageView *imageView = [_imageViewArray objectAtIndex:i];
        imageView.frame = CGRectMake(CGRectGetMaxX(lastImageView.frame) + margin, CGRectGetMinY(lastImageView.frame), H, H);
		[self addSubview:imageView];
        
        ELCOverlayImageView *overlayView = [_overlayViewArray objectAtIndex:i];
        [overlayView setFrame:imageView.frame];
