...
#define kPWDLength 6
typedef void (^ForgetPassBlock)();
typedef void (^FinishBlock)(NSString *);
@interface CYPasswordView () <UITextFieldDelegate,CYPasswordInputViewDelegate>

@property (nonatomic, copy)ForgetPassBlock forgetPassBlock;
...
-(instancetype)initWithTitle:(NSString *)title superView:(UIView *)view forgetPassBlock:(void (^)())forgetPassBlock finishBlock:(void (^)(NSString *))finishBlock{
    ...
    if (self = [super init]) {
        ...
        if (forgetPassBlock) {
            self.forgetPassBlock = forgetPassBlock;
        }
        if (finishBlock) {
            self.finishBlock = finishBlock;
        }
        [self  isShowIQKeyboardManager:NO];
...
//进入这个界面 不需要 退出时恢复
-(void)isShowIQKeyboardManager:(BOOL)isShow{
    IQKeyboardManager *manager = [IQKeyboardManager sharedManager];
    manager.enableAutoToolbar = isShow;//这个是它自带键盘工具条开关
}
- (void)dealloc{
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    if ([string isEqualToString:@""]) {
        ...
        if (tempStr.length == kPWDLength) {
            if (self.finishBlock) {
                [self  isShowIQKeyboardManager:YES];
                self.finishBlock(tempStr);
