...
- (void)layoutSubviews
{
    ...
    // 设置关闭按钮的坐标
    self.btnClose.width = 15;
    self.btnClose.height = 25;
...
- (void)setupSubViews
{
    ...
    [btnCancel setBackgroundImage:IMG(@"g_fanhui_1") forState:UIControlStateNormal];
    ...
    self.btnForgetPWD.alpha = 0;//暂时隐藏
...
- (void)btnClose_Click:(UIButton *)sender {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(btnClose_Click)]) {
        [_delegate btnClose_Click];
    }
    [self.inputNumArray removeAllObjects];
}

- (void)btnForgetPWD_Click:(UIButton *)sender {
    if (_delegate && [_delegate respondsToSelector:@selector(btnForgetPWD_Click)]) {
        [_delegate btnForgetPWD_Click];
    }
}
