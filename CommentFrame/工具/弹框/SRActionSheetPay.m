...
@interface SRActionSheetPay ()

@property (nonatomic, weak) id<SRActionSheetPayDelegate> delegate;

@property (nonatomic, copy) ActionSheetDidSelectSheetPayBlock selectSheetBlock;
...
@implementation SRActionSheetPay
...
+(instancetype)sr_actionSheetViewWithSubView:(UIView *)view{
    return [[self alloc]initWithSubView:view];
}
...
- (void)setupActionSheet {
    ...
    [self addSubview:({
        ...
        actionSheet.backgroundColor = [UIColor whiteColor];// 换成后面的颜色就有分割线 kActionSheetColor;
...
- (void)setupTitleLabel {
    ...
    if (_title && _title.length > 0) {
       ...
       [_actionSheet addSubview: titleLabel];
        ...
        UIView *contanerView  = [[UIView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(titleLabel.frame), CGRectGetWidth(titleLabel.frame), 10)];
        
        contanerView.backgroundColor = [UIColor whiteColor];
        [_actionSheet addSubview:contanerView];
        
        //支付方式的提示label
        
        UILabel *payLbl = [[UILabel alloc]initWithFrame:CGRectMake(0,0, 60, 20)];
        payLbl.center = CGPointMake(CGRectGetWidth(contanerView.frame)/2.0, CGRectGetHeight(contanerView.frame)/2.0);
        payLbl.textColor       = UIColorFromRGB(207, 207, 207);
        payLbl.textAlignment   = 1;
        payLbl.font            = [UIFont systemFontOfSize:13];
        payLbl.numberOfLines   = 0;
        payLbl.text            = @"支付方式";
        [contanerView addSubview:payLbl];
        
        UIView *lineOne = [[UIView alloc]initWithFrame:CGRectMake(20, CGRectGetHeight(payLbl.frame)/2.0 + CGRectGetMinY(payLbl.frame), CGRectGetMinX(payLbl.frame) - 30, 1)];
        lineOne.backgroundColor = UIColorFromRGB(242, 242, 242);
        [contanerView addSubview:lineOne];
        
        UIView *lineTwo = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetMaxX(payLbl.frame) + 10, CGRectGetMinY(lineOne.frame), CGRectGetWidth(lineOne.frame), 1)];
        lineTwo.backgroundColor = UIColorFromRGB(242, 242, 242);
        [contanerView addSubview:lineTwo];
        
        _offsetY += CGRectGetMaxY(contanerView.frame) + kLineHeight + 10;
    }
}
...
- (void)setOtherActionItemAlignment:(SRActionItemAlignment)otherActionItemAlignment {
    
    _otherActionItemAlignment = otherActionItemAlignment;
    
    switch (otherActionItemAlignment) {
        case SRActionItemAlignmentLeft:
        {
        ...
        case SRActionItemAlignmentCenter:
        {
            for (UIView *actionItem in self.otherActionItems) {
                UILabel *title = [actionItem viewWithTag:1];
                title.textAlignment = NSTextAlignmentCenter;
...
-(void)setSubView:(UIView *)view{
    _actionSheet = view;
    _actionSheet.frame = CGRectMake(0, CGRectGetHeight(self.frame), CGRectGetWidth(self.frame), CGRectGetHeight(view.frame));
    _actionSheetHeight = CGRectGetHeight(view.frame);
