...
@class LBPagerController;
@protocol LBPagerControllerDataSource <NSObject>
...
- (void)pagerController:(LBPagerController *)pagerController transitionFromIndex:(NSInteger)formIndex toIndex:(NSInteger)toIndex animated:(BOOL)animated;

// transition from index to index with progress
- (void)pagerController:(LBPagerController *)pagerController transitionFromIndex:(NSInteger)formIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress;

@end

@interface LBPagerController : UIViewController

@property (nonatomic, weak, readonly) UIScrollView *contentView; // don‘t change the frame

@property (nonatomic, weak) id<LBPagerControllerDataSource> dataSource;
...
@interface LBPagerController (TransitionOverride)
