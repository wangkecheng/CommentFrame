...
@interface LBTabPagerController ()<UICollectionViewDelegateFlowLayout, UICollectionViewDataSource>
{
    struct {
...
@implementation LBTabPagerController
...
- (void)setDataSource:(id<LBPagerControllerDataSource>)dataSource
{
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (_cellWidth > 0) {
        ...
        //这是自己定义获取pager回调方法，若是pager个数过多，可能会出现所有pager选项都在一个界面的尴尬情况，此时就可以把width作为作为lastWidth，而废弃掉equleWidth
        NSInteger pagerCount = 0;
        if (_pagerCountDelegate!=nil && [_pagerCountDelegate respondsToSelector:@selector(getPagerCount)]) {
            pagerCount = [_pagerCountDelegate getPagerCount];
        }
        //用屏幕宽度除以总pager选项数，得到pager选项的宽度
        CGFloat  equleWidth = [UIScreen mainScreen].bounds.size.width/pagerCount;
        
        //根据pager选项字符宽度设置pager选项宽度
        ...
        CGFloat  lastWidth = width > equleWidth? width:equleWidth;
        
        return CGSizeMake(lastWidth, CGRectGetHeight(_collectionViewBar.frame));
    }
    return CGSizeZero;
}
